...
export default function HouseRow({
  h, areaName, paid, avatarUrl, onClick,
}: { h: House; areaName?: string; paid: boolean; avatarUrl?: string; onClick: () => void }) {
  return (
      ...
      <div className="min-w-0 flex items-center gap-2">
        {avatarUrl ? (
          <span className="relative shrink-0">
            <img src={avatarUrl} alt="" className="w-8 h-8 rounded-full object-cover border border-stone-200" />
            <span className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full ring-2 ring-white ${paid ? 'bg-green-500' : 'bg-stone-300'}`}
