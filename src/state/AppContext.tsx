...
import type { Finance, Membership, Perm, Profile } from '../lib/types';
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    setMemberships(list);
    setPid((prev) => {
      if (prev && list.some((m) => m.program_id === prev)) return prev;
      const first = list[0]?.program_id ?? null;
  ...
  const frozen = current?.programs?.status === 'frozen';
  ...
  return (
    <Ctx.Provider
      value={{
        session, profile, memberships, currentProgramId, setCurrentProgramId,
        current, finance, loading, isPadmin, frozen, can, isCommitteeAdmin,
